  ...
  *builtin_data = nullptr;
  switch (op_type) {
    ...
    case BuiltinOperator_LSH_PROJECTION: {
      ...
      TFL_MIGRATION_ENSURE(params != nullptr);
      ...
      if (unique_params != nullptr) {
        params->index_out_type =
            unique_params->idx_out_type() == tflite::TensorType_INT64
                ? TFLMigrationType::kTfLiteInt64
                : TFLMigrationType::kTfLiteInt32;
      }
      *builtin_data = params.release();
      return OkStatus();
    }
    ...
    case BuiltinOperator_HASHTABLE: {
      ...
      if (const auto* hashtable_params =
        ...
        TFL_MIGRATION_ENSURE_STATUS(ConvertTensorType(
            hashtable_params->key_dtype(), &params->key_dtype));
        TFL_MIGRATION_ENSURE_STATUS(ConvertTensorType(
...
absl::Status ConvertTensorType(TensorType tensor_type, TFLMigrationType* type) {
  switch (tensor_type) {
...
absl::Status ParseCast(const Operator* op, BuiltinDataAllocator* allocator,
  ...
  if (const auto* schema_params = op->builtin_options_as_CastOptions()) {
    TFL_MIGRATION_ENSURE_STATUS(ConvertTensorType(schema_params->in_data_type(),
                                                  &params->in_data_type));
    TFL_MIGRATION_ENSURE_STATUS(ConvertTensorType(
        schema_params->out_data_type(), &params->out_data_type));
  }
  *builtin_data = params.release();
  return OkStatus();
}
...
absl::Status ParseReshape(const Operator* op, BuiltinDataAllocator* allocator,
  ...
  if (schema_params != nullptr) {
    ...
    if (new_shape != nullptr) {
      TFL_MIGRATION_ENSURE_STATUS(FlatBufferIntVectorToArray(
  ...
  if (schema_params) {
    ...
    auto LoadAttr = [](int64_t* params_array, size_t params_array_size_bytes,
      ...
      if (flatbuffer_vector && flatbuffer_vector->size()) {
        ...
        if (!status.ok()) {
          auto error_message =
              absl::StrFormat("Check the '%s' attribute.", attr_name);
          LOG(ERROR) << error_message;
          return absl::InvalidArgumentError(error_message);
        }
      } else {
    ...
    TFL_MIGRATION_ENSURE_STATUS(
        LoadAttr(params->window_dilations, sizeof(params->window_dilations),
                 ...
                 /*expected_size=*/rank, /*fill_value=*/1));
    TFL_MIGRATION_ENSURE_STATUS(LoadAttr(params->padding,
...
absl::Status ParseStablehloScatter(const Operator* op,
  ...
  if (schema_params) {
    ...
    if (schema_params->update_window_dims()) {
      TFL_MIGRATION_ENSURE_STATUS(FlatBufferIntVectorToArray<int64_t>(
...
absl::Status ParseStablehloPad(const Operator* op,
  ...
  if (schema_params) {
    auto LoadAttr =
      ...
      if (!status.ok()) {
        LOG(ERROR) << absl::StrFormat("Check the '%s' attribute.", attr_name);
      }
      return status;
    };
