...
class QuantizeConcatResult : public OpRewritePattern<TF::ConcatV2Op> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::ConcatV2Op concat,
    ...
    for (auto &use : concat.getOutput().getUses()) {
      if (mlir::dyn_cast_or_null<TFL::QuantizeOp>(use.getOwner()) ||
          mlir::dyn_cast_or_null<TF::FakeQuantWithMinMaxVarsOp>(
              use.getOwner())) {
        return failure();
      }
    }
    ...
    double min = std::numeric_limits<bool>::max();
    double max = std::numeric_limits<bool>::min();
    Value min_v;
    Value max_v;
    ...
    for (TF::FakeQuantWithMinMaxVarsOp fq : fake_quant_ops) {
      ...
      if (auto new_min = FloatAttr::getValueAsDouble(*min_attr.begin());
          new_min <= min) {
        min = new_min;
        min_v = fq.getMin();
      }
      if (auto new_max = FloatAttr::getValueAsDouble(*max_attr.begin());
          new_max >= max) {
        max = new_max;
        max_v = fq.getMax();
      }
