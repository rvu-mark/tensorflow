...
def _create_fake_libcudalibrary_and_symlinks(
        repository_ctx,
        lib_name_to_version_dict):
    if repository_ctx.name == "cuda_driver":
        ...
        repository_ctx.file(
            "libcuda.cc",
            repository_ctx.read(repository_ctx.attr.fake_libcuda),
        )
...
def _use_downloaded_cuda_redistribution(repository_ctx):
    ...
    _create_fake_libcudalibrary_and_symlinks(
        repository_ctx,
        lib_name_to_version_dict,
    )
    repository_ctx.file("version.txt", major_version)
...
cuda_repo = repository_rule(
    ...
    attrs = {
        ...
        "fake_libcuda": attr.label(default = Label(
            "//third_party/gpus/cuda/hermetic:libcuda.cc",
        )),
    },
    environ = [
